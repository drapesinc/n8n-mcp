...
exports.scanWorkflows = scanWorkflows;
const credential_scanner_1 = require("./credential-scanner");
function checkHardcodedSecrets(workflow) {
    const detections = (0, credential_scanner_1.scanWorkflow)({
        id: workflow.id,
        name: workflow.name,
...
function checkUnauthenticatedWebhooks(workflow) {
    const findings = [];
    let sequence = 0;
...
function checkErrorHandlingGaps(workflow) {
    if (workflow.nodes.length < 3) {
        return [];
    }
    const hasContinueOnFail = workflow.nodes.some((node) => node.continueOnFail === true);
    const hasOnErrorHandling = workflow.nodes.some((node) => typeof node.onError === 'string' && node.onError !== 'stopWorkflow');
    const hasErrorTrigger = workflow.nodes.some((node) => (node.type ?? '').toLowerCase() === 'n8n-nodes-base.errortrigger');
...
function checkDataRetentionSettings(workflow) {
    const settings = workflow.settings;
    if (!settings) {
        return [];
    }
    const savesAllData = settings.saveDataErrorExecution === 'all' &&
...
const ALL_CHECKS = Object.keys(CHECK_MAP);
function scanWorkflows(workflows, checks) {
...
//# sourceMappingURL=workflow-security-scanner.js.map